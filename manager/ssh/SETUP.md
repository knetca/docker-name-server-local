...
`id_rsa` is gitignored — never committed on either branch.
`known_hosts` is safe to commit on the private branch.

## 1. Generate the deploy key

```bash
ssh-keygen -t rsa -b 4096 -C "dns-manager@$(hostname)" \
...
```bash
ssh-keyscan github.com ssh.dev.azure.com > manager/ssh/known_hosts
```
